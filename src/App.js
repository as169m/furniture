...
import { Ruler, LayoutDashboard, Box, DollarSign, Hammer, Percent, ChevronDown, ChevronUp, CheckCircle2, XCircle, Lightbulb, Palette, GlassWater } from 'lucide-react';
...
const MATERIAL_COSTS_PER_SQFT = {
    ...
    mdf_12mm: 2.00,
    mdf_6mm: 1.50,
    solid_wood_18mm: 10.00,
    solid_wood_12mm: 7.00,
    solid_wood_6mm: 4.00,
};

const EDGE_BANDING_COST_PER_LINEAR_FT = 0.50;
...
const HARDWARE_OPTIONS = {
    ...
    hanging_rod: 15.00, // Still a single type for simplicity
};

const LIGHTING_COST_PER_FT = 10.00; // Cost per linear foot of lighting
const GLASS_DOOR_ADDITIONAL_COST_PER_DOOR = 30.00; // Additional cost for glass per door
...
const CURRENCY_RATES = {
    ...
    INR: 83.50, // Example: 1 USD = 83.50 INR (adjust as needed)
};
...
function App() {
    ...
    // New: Custom rates
    const [customLaborRate, setCustomLaborRate] = useState(30.00); // USD per hour
    ...
    const validateInputs = () => {
        ...
        if (numDrawers > 0 && drawerHeight <= 0) newErrors.drawerHeight = 'Must be positive for drawers.';
        if (numDrawers > 0 && drawerDepth <= 0) newErrors.drawerDepth = 'Must be positive for drawers.';
        if (customLaborRate < 0) newErrors.customLaborRate = 'Cannot be negative.';
        if (customMarkupPercentage < 0) newErrors.customMarkupPercentage = 'Cannot be negative.';
        if (customMarkupPercentage > 100) newErrors.customMarkupPercentage = 'Cannot exceed 100%.';
    ...
    useEffect(() => {
        ...
        height, width, depth, materialType, numShelves, numDrawers, hasHangingRod, hasBackPanel,
        numDoors, drawerHeight, drawerDepth, customLaborRate, customMarkupPercentage,
        hingeType, drawerSlideType, handleType, hasLighting, finishType, hasGlassDoors,
    ...
    const calculateCosts = () => {
        ...
        materialCost18mm += inchesToSqFeet(sidesAreaSqIn + topBottomAreaSqIn) * material18mmPrice;
        ...
        if (numDoors > 0) {
            ...
            edgeBandingLengthFt += inchesToFeet(numDoors * (2 * doorWidth + 2 * height)); // All 4 edges of each door
        ...
        if (numDrawers > 0) {
            ...
            const drawerBoxAreaSqIn = numDrawers * (
            ...
            materialCost18mm += inchesToSqFeet(drawerFrontAreaSqIn) * material18mmPrice;

            edgeBandingLengthFt += inchesToFeet(numDrawers * (2 * avgDrawerWidth + 2 * drawerHeight)); // All 4 edges of drawer front
        ...
        // --- Cost Calculation ---
        const calculatedTotalMaterialCost = materialCost18mm + materialCost12mm + materialCost6mm; // Sum of all material costs
        const calculatedEdgeBandingCost = edgeBandingLengthFt * EDGE_BANDING_COST_PER_LINEAR_FT;

        let calculatedHardwareCost = 0;
        calculatedHardwareCost += hardwareCount.hinge * HARDWARE_OPTIONS.hinge[hingeType];
        calculatedHardwareCost += hardwareCount.drawer_slide * HARDWARE_OPTIONS.drawer_slide[drawerSlideType];
        calculatedHardwareCost += hardwareCount.handle * HARDWARE_OPTIONS.handle[handleType];
        calculatedHardwareCost += hardwareCount.hanging_rod * HARDWARE_OPTIONS.hanging_rod;
        ...
        if (finishType !== 'none') {
            ...
            const exteriorAreaSqIn = (2 * height * depth) + (width * height) + (width * depth);
            featuresCost += inchesToSqFeet(exteriorAreaSqIn) * FINISH_COSTS_PER_SQFT_EXTERIOR[finishType];
        }

        const subTotalCost = calculatedTotalMaterialCost + calculatedEdgeBandingCost + calculatedHardwareCost + calculatedLaborCost + featuresCost;
        const finalCalculatedCost = subTotalCost * (1 + (customMarkupPercentage / 100));
    ...
    const renderInputField = (id, label, value, onChange, type = 'number', min = 0, max = Infinity) => (
        <div>
            ...
            <input
                ...
                id={id}
                value={value}
                onChange={(e) => onChange(type === 'number' ? Math.max(min, Math.min(max, parseFloat(e.target.value) || 0)) : e.target.value)}
                className={`w-full p-3 border rounded-md focus:ring-blue-500 focus:border-blue-500 transition duration-200 ${errors[id] ? 'border-red-500' : 'border-gray-300'}`}
                min={min}
                max={max}
            />
            ...
            >
                {Object.entries(options).map(([key, val]) => (
                    <option key={key} value={key}>
                        ...
                        {typeof val === 'number' && ` (${CURRENCY_SYMBOLS['USD']}${val.toFixed(2)})`} {/* Show USD base cost for options */}
                    </option>
                ))}
            </select>
        </div>
        ...
        <div className="min-h-screen bg-gradient-to-br from-gray-100 to-gray-200 font-sans text-gray-800 p-4 sm:p-6">
            <div className="max-w-4xl mx-auto bg-white shadow-xl rounded-xl overflow-hidden">
                <header className="bg-blue-600 text-white p-6 flex items-center justify-center rounded-t-xl">
                    ...
                    <h1 className="text-3xl font-bold text-center">Wardrobe Cost Estimator</h1>
                </header>
                ...
                <div className="p-6 sm:p-8 space-y-8">
                    ...
                    <section className="bg-blue-50 p-6 rounded-lg shadow-inner">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
                            {renderInputField('height', 'Height (inches)', height, setHeight, 'number', 1)}
                            {renderInputField('width', 'Width (inches)', width, setWidth, 'number', 1)}
                            {renderInputField('depth', 'Depth (inches)', depth, setDepth, 'number', 1)}
                        </div>

                        <div className="mb-6">
                            <label htmlFor="materialType" className="block text-sm font-medium text-gray-700 mb-1">Main Material</label>
                            <select
                                id="materialType"
                                value={materialType}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {renderInputField('numShelves', 'Number of Shelves', numShelves, setNumShelves, 'number', 0)}
                            {renderInputField('numDrawers', 'Number of Drawers', numDrawers, setNumDrawers, 'number', 0)}
                            {numDrawers > 0 && (
                                <>
                                    {renderInputField('drawerHeight', 'Avg. Drawer Height (inches)', drawerHeight, setDrawerHeight, 'number', 1)}
                                    {renderInputField('drawerDepth', 'Avg. Drawer Depth (inches)', drawerDepth, setDrawerDepth, 'number', 1)}
                                </>
                            )}
                            {renderInputField('numDoors', 'Number of Doors', numDoors, setNumDoors, 'number', 0)}
                            <div className="flex items-center mt-4">
                                ...
                                />
                                <label htmlFor="hasHangingRod" className="ml-2 block text-sm font-medium text-gray-700">Include Hanging Rod</label>
                            </div>
                            <div className="flex items-center mt-4">
                                <input
                                    type="checkbox"
                                    ...
                                    className="h-5 w-5 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                                />
                                <label htmlFor="hasBackPanel" className="ml-2 block text-sm font-medium text-gray-700">Include Back Panel</label>
                            </div>
                        </div>
                    </section>
                    ...
                    <section className="bg-purple-50 p-6 rounded-lg shadow-inner">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            {renderSelectField('hingeType', 'Hinge Type', hingeType, setHingeType, HARDWARE_OPTIONS.hinge)}
                            {renderSelectField('drawerSlideType', 'Drawer Slide Type', drawerSlideType, setDrawerSlideType, HARDWARE_OPTIONS.drawer_slide)}
                            {renderSelectField('handleType', 'Handle Type', handleType, setHandleType, HARDWARE_OPTIONS.handle)}
                        </div>
                    </section>
                    ...
                    <section className="bg-yellow-50 p-6 rounded-lg shadow-inner">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="flex items-center">
                                ...
                                />
                                <label htmlFor="hasLighting" className="ml-2 block text-sm font-medium text-gray-700">Include Lighting</label>
                            </div>
                            {renderSelectField('finishType', 'Exterior Finish', finishType, setFinishType, FINISH_COSTS_PER_SQFT_EXTERIOR)}
                            <div className="flex items-center">
                                <input
                                    type="checkbox"
                                    id="hasGlassDoors"
                                    ...
                                    className="h-5 w-5 text-yellow-600 border-gray-300 rounded focus:ring-yellow-500"
                                />
                                <label htmlFor="hasGlassDoors" className="ml-2 block text-sm font-medium text-gray-700">Glass Doors</label>
                            </div>
                        </div>
                    </section>
                    ...
                    <section className="bg-indigo-50 p-6 rounded-lg shadow-inner">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            {renderInputField('customLaborRate', `Labor Hourly Rate (${CURRENCY_SYMBOLS['USD']}/hr)`, customLaborRate, setCustomLaborRate, 'number', 0)}
                            {renderInputField('customMarkupPercentage', 'Markup Percentage (%)', customMarkupPercentage, setCustomMarkupPercentage, 'number', 0, 100)}
                            <div>
                    ...
                    <section className="bg-green-50 p-6 rounded-lg shadow-inner">
                        ...
                        <div className="space-y-3 text-lg">
                            <div className="flex justify-between items-center py-2 border-b border-green-200">
                                <span className="font-medium text-gray-700">Total Material Cost:</span>
                                <span className="font-bold text-green-700">{formatCurrency(totalMaterialCost)}</span>
                            </div>
                            ...
                            {materialCost18mmDisplay > 0 && (
                                <div className="flex justify-between items-center py-1 pl-4 text-sm text-gray-600">
                                    <span>- {materialType.replace('_', ' ').split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')} 18mm:</span>
                                    <span>{formatCurrency(materialCost18mmDisplay)}</span>
                                </div>
                            )}
                            {materialCost12mmDisplay > 0 && (
                                <div className="flex justify-between items-center py-1 pl-4 text-sm text-gray-600">
                                    <span>- {materialType.replace('_', ' ').split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')} 12mm:</span>
                                    <span>{formatCurrency(materialCost12mmDisplay)}</span>
                                </div>
                            )}
                            {materialCost6mmDisplay > 0 && (
                                <div className="flex justify-between items-center py-1 pl-4 text-sm text-gray-600">
                                    <span>- {materialType.replace('_', ' ').split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')} 6mm:</span>
                                    <span>{formatCurrency(materialCost6mmDisplay)}</span>
                                </div>
                            )}

                            <div className="flex justify-between items-center py-2 border-b border-green-200">
                                <span className="font-medium text-gray-700">Edge Banding Cost:</span>
                                <span className="font-bold text-green-700">{formatCurrency(totalEdgeBandingCost)}</span>
                            </div>
                            <div className="flex justify-between items-center py-2 border-b border-green-200">
                                <span className="font-medium text-gray-700">Hardware Cost:</span>
                                <span className="font-bold text-green-700">{formatCurrency(totalHardwareCost)}</span>
                            </div>
                            <div className="flex justify-between items-center py-2 border-b border-green-200">
                                <span className="font-medium text-gray-700">Estimated Labor Hours:</span>
                                <span className="font-bold text-green-700">{estimatedLaborHours.toFixed(1)} hrs</span>
                            </div>
                            <div className="flex justify-between items-center py-2 border-b border-green-200">
                                <span className="font-medium text-gray-700">Estimated Labor Cost:</span>
                                <span className="font-bold text-green-700">{formatCurrency(estimatedLaborCost)}</span>
                            </div>
                            <div className="flex justify-between items-center py-2 border-b border-green-200">
                                <span className="font-medium text-gray-700">Additional Features Cost:</span>
                                <span className="font-bold text-green-700">{formatCurrency(additionalFeaturesCost)}</span>
                            </div>
                            <div className="flex justify-between items-center py-2 border-b-2 border-green-400 font-bold text-xl text-green-900">
                                <span>Subtotal:</span>
                                <span>{formatCurrency(totalMaterialCost + totalEdgeBandingCost + totalHardwareCost + estimatedLaborCost + additionalFeaturesCost)}</span>
                            </div>
                            <div className="flex justify-between items-center py-2 font-bold text-2xl text-green-900">
                                <span className="flex items-center">
                                    <Percent className="w-6 h-6 mr-2" /> Total Estimated Cost (incl. {customMarkupPercentage}% Markup):
                                </span>
                                <span className="text-green-900">{formatCurrency(finalCost)}</span>
                            </div>
                        </div>
                    </section>
                    ...
                    {showDetails && (
                        <section className="bg-gray-50 p-6 rounded-lg shadow-inner mt-6">
                            ...
                            <div className="space-y-4 text-gray-700">
                                <p><strong>Wardrobe Dimensions:</strong> {height}" H x {width}" W x {depth}" D</p>
                                <p><strong>Main Material:</strong> {materialType.replace('_', ' ').split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')}</p>
                                <p><strong>Number of Shelves:</strong> {numShelves}</p>
                                <p><strong>Number of Drawers:</strong> {numDrawers}</p>
                                {numDrawers > 0 && (
                                    <>
                                        <p><strong>Avg. Drawer Height:</strong> {drawerHeight}"</p>
                                        <p><strong>Avg. Drawer Depth:</strong> {drawerDepth}"</p>
                                    </>
                                )}
                                <p><strong>Number of Doors:</strong> {numDoors}</p>
                                <p className="flex items-center">
                                    <strong>Hanging Rod:</strong> {hasHangingRod ? <CheckCircle2 className="w-5 h-5 text-green-500 ml-2" /> : <XCircle className="w-5 h-5 text-red-500 ml-2" />}
                                </p>
                                <p className="flex items-center">
                                    <strong>Back Panel:</strong> {hasBackPanel ? <CheckCircle2 className="w-5 h-5 text-green-500 ml-2" /> : <XCircle className="w-5 h-5 text-red-500 ml-2" />}
                                </p>
                                <p><strong>Hinge Type:</strong> {hingeType.replace('_', ' ').split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')}</p>
                                <p><strong>Drawer Slide Type:</strong> {drawerSlideType.replace('_', ' ').split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')}</p>
                                <p><strong>Handle Type:</strong> {handleType.replace('_', ' ').split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')}</p>
                                <p className="flex items-center">
                                    <strong>Lighting:</strong> {hasLighting ? <CheckCircle2 className="w-5 h-5 text-green-500 ml-2" /> : <XCircle className="w-5 h-5 text-red-500 ml-2" />}
                                </p>
                                <p><strong>Exterior Finish:</strong> {finishType.replace('_', ' ').split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')}</p>
                                <p className="flex items-center">
                                    <strong>Glass Doors:</strong> {hasGlassDoors ? <CheckCircle2 className="w-5 h-5 text-green-500 ml-2" /> : <XCircle className="w-5 h-5 text-red-500 ml-2" />}
                                </p>
                                <p><strong>Custom Labor Rate:</strong> {formatCurrency(customLaborRate)}/hr (Base USD rate: ${customLaborRate.toFixed(2)}/hr)</p>
                                <p><strong>Custom Markup:</strong> {customMarkupPercentage}%</p>
                                ...
                                <div className="mt-4 pt-4 border-t border-gray-200">
                                    ...
                                    <ul className="list-disc list-inside text-sm space-y-1">
                                        <li>All base costs (materials, hardware, additional features) are defined in US Dollars ($).</li>
                                        <li>The final display currency is based on the selected exchange rate.</li>
                                        <li>Material thickness costs are applied based on component: 18mm for main panels/doors/drawer fronts, 12mm for shelves/drawer boxes, 6mm for back panel.</li>
                                        <li>Drawer dimensions ({drawerHeight}" H x {drawerDepth}" D) are estimates for material calculation.</li>
                                        <li>Labor hours are estimated based on complexity and may vary.</li>
                                        <li>Costs do not include delivery, installation, or any custom finishes unless specified.</li>
                                        <li>All dimensions are in inches for input, converted to feet for calculations.</li>
                                        <li>Markup percentage is applied to the subtotal (materials + hardware + labor + additional features).</li>
                                    </ul>
                                </div>
                            </div>
                        </section>
                ...
                <footer className="bg-blue-600 text-white p-4 text-center text-sm rounded-b-xl">
                    <p>&copy; {new Date().getFullYear()} Furniture Cost Estimator. All rights reserved.</p>
                </footer>
            </div>
        </div>
    );
